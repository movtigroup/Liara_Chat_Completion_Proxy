...
class CompletionRequest(BaseModel):
    model: str = Field(..., description="مدل هوش مصنوعی مورد استفاده")
    messages: List[Message] = Field(..., min_length=1, description="لیست پیام‌های گفتگو")

    max_tokens: Optional[int] = Field(None, gt=1, description="حداکثر توکن خروجی")
    temperature: Optional[float] = Field(None, ge=0.0, le=2.0, description="درجه خلاقیت")
    ...
    frequency_penalty: Optional[float] = Field(None, ge=-2.0, le=2.0, description="جریمه تکرار")
    presence_penalty: Optional[float] = Field(None, ge=-2.0, le=2.0, description="جریمه حضور")
